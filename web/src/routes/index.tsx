...
function LandingPage() {
  ...
  const { count: connectionCount, countRef: connectionRef } = useCountUp(15, 2000);
  const { count: queryCount, countRef: queryRef } = useCountUp(100, 2500);
      ...
      <main>
        ...
        <section className="relative">
          ...
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            <FloatingIcon 
              icon={Database} 
              className="w-16 h-16 top-20 left-[10%] text-primary animate-float" 
              delay={0}
            />
            <FloatingIcon 
              icon={Code2} 
              className="w-12 h-12 top-40 right-[15%] text-primary animate-float-reverse" 
              delay={1}
            />
            <FloatingIcon 
              icon={Terminal} 
              className="w-20 h-20 bottom-32 left-[20%] text-primary animate-float-slow" 
              delay={2}
            />
            <FloatingIcon 
              icon={Table} 
              className="w-14 h-14 top-60 left-[5%] text-primary animate-float" 
              delay={1.5}
            />
            <FloatingIcon 
              icon={Zap} 
              className="w-10 h-10 bottom-40 right-[10%] text-primary animate-float-reverse" 
              delay={0.5}
            />
          </div>
          ...
          <div className="container mx-auto px-4 pt-20 md:pt-32 pb-16 relative z-10">
            <div 
              ref={heroRef.ref}
              className={`max-w-3xl transition-all duration-1000 ${heroRef.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
            >
              {/* Badge */}
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20 text-sm mb-6 animate-fade-in-up">
                <Sparkles className="w-4 h-4" />
                <span>Now with AI-powered queries</span>
              </div>
              ...
              <div className="mt-10 flex items-center gap-4 animate-fade-in-up stagger-3">
                <Button 
                  asChild 
                  size="lg" 
                  className="h-12 px-8 text-base animate-pulse-glow hover:scale-105 transition-transform"
                >
                  <Link to="/download">
        ...
        <section 
          ref={videoRef.ref}
          className={`container mx-auto px-4 pb-24 transition-all duration-1000 delay-300 ${videoRef.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
        >
        ...
        <section 
          ref={statsRef.ref}
          className={`border-y bg-muted/20 transition-all duration-1000 ${statsRef.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
        >
          <div className="container mx-auto px-4 py-16">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
              <div className="space-y-2">
                <span className="text-4xl font-bold" ref={connectionRef}>
                  {connectionCount}+
                </span>
                <div className="text-sm text-muted-foreground">Database Types</div>
              </div>
              <div className="space-y-2">
                <span className="text-4xl font-bold" ref={queryRef}>
                  {queryCount}K+
                </span>
        ...
        <section className="border-t bg-muted/30">
          <div 
            ref={featuresRef.ref}
            className={`container mx-auto px-4 py-24 transition-all duration-1000 ${featuresRef.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
          >
            ...
            <div className="grid md:grid-cols-3 gap-8">
              <div className="group relative p-8 rounded-2xl bg-background border hover-glow card-shine transition-all duration-300">
                ...
                <p className="text-muted-foreground leading-relaxed">
                  PostgreSQL & MySQL support with SQLite, MongoDB and Redis coming soon. All local-first and secure.
                </p>
              ...
              <div className="group relative p-8 rounded-2xl bg-background border hover-glow card-shine transition-all duration-300">
                ...
                <p className="text-muted-foreground leading-relaxed">
                  Syntax highlighting, auto-complete, and keyboard shortcuts. Run queries with ⌘↵ and copy results directly.
                </p>
                <div className="mt-6 flex items-center gap-2 text-sm text-primary opacity-0 group-hover:opacity-100 transition-opacity">
                  <span>Learn more</span>
                  <ArrowRight className="w-4 h-4" />
              ...
              <div className="group relative p-8 rounded-2xl bg-background border hover-glow card-shine transition-all duration-300">
                ...
                <p className="text-muted-foreground leading-relaxed">
                  A native AI agent built into your database. Describe what you need in plain English and it writes SQL based on your schema.
                </p>
                <div className="mt-6 flex items-center gap-2 text-sm text-primary opacity-0 group-hover:opacity-100 transition-opacity">
                  <span>Learn more</span>
                  <ArrowRight className="w-4 h-4" />
        ...
        <section className="container mx-auto px-4 py-24">
          <div className="max-w-4xl mx-auto text-center">
            <div className="relative p-12 rounded-3xl bg-gradient-to-br from-primary/5 via-background to-accent/5 border">
              ...
              <div className="relative z-10">
                ...
                <p className="text-muted-foreground text-lg mb-8 max-w-xl mx-auto">
                  Join thousands of developers who've switched to QueryStudio for their database management needs.
                </p>
                <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                  <Button 
                    asChild 
                    size="lg" 
                    className="h-12 px-8 animate-pulse-glow"
                  >
                    <Link to="/download">
                      <Download className="h-5 w-5 mr-2" />
                      Download for Free
